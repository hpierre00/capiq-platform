...
async function hmacKey(): Promise<CryptoKey> {
  ...
  const derived = await crypto.subtle.sign('HMAC', rk, new TextEncoder().encode('capiq-realtor-legacy-token-v1'));
...
serve(async (req) => {
  ...
  try {
    ...
    if (action === 'login') {
      ...
      return R({ success: true, token: await gt(r.id, r.email), realtor: sf(r) });
    }
    if (action === 'signup') {
    ...
    if (action === 'verify') {
      ...
      const p = await vt(token); if (p) { const { data: r } = await sb.from('realtor_users').select('*').eq('id', p.id).maybeSingle(); if (r) return new Response(JSON.stringify({ valid: true, realtor: sf(r) }), { status: 200, headers: cors }); }
    ...
    if (action === 'increment_usage') {
      ...
      if (token?.split('.').length === 3) { const { data: { user } } = await sa.auth.getUser(token); if (user) { const { data: r } = await sb.from('realtor_users').select('id').eq('email', user.email).maybeSingle(); if (r) rid = r.id; } } else { const p = await vt(token); if (p) rid = p.id; }
...
async function gt(id: string, email: string): Promise<string> { const p = { id, email, role: 'realtor', iat: Date.now(), exp: Date.now() + TTL }; const d = btoa(JSON.stringify(p)); return d + '.' + await hm(d); }
async function vt(t: string): Promise<any> { try { if (!t) return null; const [d, s] = t.split('.'); if (await hm(d) !== s) return null; const p = JSON.parse(atob(d)); return p.exp < Date.now() ? null : p; } catch { return null; } }
async function hm(d: string): Promise<string> { const key = await hmacKey(); const sig = await crypto.subtle.sign('HMAC', key, new TextEncoder().encode(d)); return Array.from(new Uint8Array(sig)).map(b => b.toString(16).padStart(2, '0')).join(''); }
