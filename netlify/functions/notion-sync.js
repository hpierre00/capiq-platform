// Syncs analyzed deal to Notion Deal Pipeline
export default async (req) => {
  ...
  try {
    ...
    const dealTypeMap = { 'Purchase': 'Fix & Flip', 'Fix & Flip': 'Fix & Flip', 'Rental': 'Rental / DSCR', 'Rental/DSCR': 'Rental / DSCR', 'Cash-Out': 'Cash-Out', 'Cash-Out Refi': 'Cash-Out', 'Bridge': 'Bridge', 'New Construction': 'Fix & Flip' };
    const propTypeMap = { 'SFR': 'SFR', 'Condo': 'SFR', '2-4 Unit': '2-4 Unit', 'Multifamily 5+': 'Commercial', 'Commercial': 'Commercial', 'Land': 'Commercial' };
    const expMap = { 'First Deal': 'First Deal', '1-3 Deals': '1-3 Deals', '4-10 Deals': '4-10 Deals', '10-20 Deals': '10+ Deals', '20+ Deals': '10+ Deals' };
    const scoreGrade = (s) => !s ? 'Review' : s >= 70 ? 'Pass' : s >= 45 ? 'Review' : 'Reject';
    ...
    const props = {
      ...
      "Human Review Required": { checkbox: !!(a.humanReviewRequired) },
      "Intake Date": { date: { start: today } },  // correct REST API format
    };
    ...
    if (d.notes) props["Notes"] = { rich_text: [{ text: { content: String(d.notes).slice(0, 2000) } }] };
    if (a.fundabilityScore) {
      props["Fundability Score"] = { number: a.fundabilityScore };
      props["Deal Score"] = { select: { name: scoreGrade(a.fundabilityScore) } };
    }
    ...
    if (a.scoreBreakdown) props["Score Breakdown"] = { rich_text: [{ text: { content: String(a.scoreBreakdown).slice(0, 2000) } }] };
    ...
    if (!res.ok) { console.error("Notion error:", JSON.stringify(result)); throw new Error(result.message || "Notion API error"); }
    return new Response(JSON.stringify({ success: true, notionPageId: result.id }), { status: 200, headers: cors });
