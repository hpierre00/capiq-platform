...
const BUILD = "2026-09-10-deal-capture-bg-fn";
...
{"fundabilityScore":0,"dealScore":"Pass","humanReviewRequired":false,"executiveSummary":"","strengthsAndRisks":"","lenderMatchingProfile":"","structuringRecommendations":"","marketContext":"","scoreBreakdown":"","nextSteps":""}`;
    ...
    // Persist the deal in a Netlify BACKGROUND function (its own 15-min budget)
    // rather than here: after the Claude call there is not enough of this
    // function's 30s left for six sequential Supabase round-trips, and
    // context.waitUntil() work shares that same wall clock. Fire-and-forget,
    // same as emailTask / notionTask — a failed invoke never touches the
    // analysis response. The background function does the investor-token
    // resolution, borrower/deal/scores/matches/events writes, and its own
    // stage-tracked failure logging + deal_save_failed event.
    const saveTask = fetch(`${baseUrl}/.netlify/functions/capiq-save-deal-background`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "x-internal-key": Netlify.env.get("SUPABASE_SERVICE_KEY") || "" },
      body: JSON.stringify({ dealData: d, analysis, token: body.token || null }),
    }).catch(() => {});

    // Registered, not awaited: the client gets the analysis result now; Netlify
    // keeps this invocation alive in the background only long enough to fire the
    // three side-effect requests (each returns fast — the save is a background
    // function that 202s immediately and finishes on its own budget). Each task
    // swallows its own errors, so a failed side effect can't surface here.
    if (context && typeof context.waitUntil === "function") {
      context.waitUntil(Promise.allSettled([emailTask, notionTask, saveTask]));
    }
