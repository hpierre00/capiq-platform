...
const BUILD = "2026-09-10-deal-capture-consolidation";
...
{"fundabilityScore":0,"dealScore":"Pass","humanReviewRequired":false,"executiveSummary":"","strengthsAndRisks":"","lenderMatchingProfile":"","structuringRecommendations":"","marketContext":"","scoreBreakdown":"","nextSteps":""}`;
    ...
    //
    // Consolidated deal-capture write path (spec 2026-09-10). Runs only here,
    // only in the deferred waitUntil block. capiq-save-deal (the old parallel
    // path) no longer has a caller. Every form value that lands in a
    // CHECK-constrained column goes through a mapper in ./lib/deal-mappers.mjs.
    const supabaseTask = SVC_KEY ? (async () => {
      ...
      try {
        // 1. Resolve the investor from the session token — deferred, never on
        //    the response critical path (30s Netlify ceiling). Any failure or a
        ...
        if (tok && tok.split('.').length === 3) {
          const ur = await fetch(`${SUPABASE_URL}/auth/v1/user`, {
            headers: { apikey: SVC_KEY, Authorization: `Bearer ${tok}` },
          });
          if (ur.ok) {
        ...
        // 6. Insert lender_matches. match_status must be matched/conditional/
        //    rejected — the prior 'pending' value violated the CHECK (23514).
        stage = 'matches';
        if (routable.length) {
